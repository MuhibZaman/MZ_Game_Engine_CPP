...
namespace lve {
    ...
    void LveCamera::setViewDirection(glm::vec3 position, glm::vec3 direction, glm::vec3 up) {
        //Constructs orthonormal basis (3 vectors all unit length and orthogonal to each other)
        const glm::vec3 w{glm::normalize(direction)};
        const glm::vec3 u{glm::normalize(glm::cross(w, up))};
        const glm::vec3 v{glm::cross(w, u)};

        //Basis vectors used as rotation matrix that is combined with translation matrix rom cameras position back to origin
    ...
    void LveCamera::setViewTarget(glm::vec3 position, glm::vec3 target, glm::vec3 up) {
        assert(target != position && "target and position cannot be the same point");
    ...
    void LveCamera::setViewYXZ(glm::vec3 position, glm::vec3 rotation) {
        ...
        //Combines inverse of rotation matrix with translation back to origin
        viewMatrix = glm::mat4{1.f};
        viewMatrix[0][0] = u.x;
        viewMatrix[1][0] = u.y;
        viewMatrix[2][0] = u.z;
